 ...
 * S012AckHandler
 * <p>
 * 負責處理 CMD_ID=S012 的 ACK（安全門關閉請求的回覆）。
 *
 * 職責：
...
@Slf4j
@Component
public class S012AckHandler extends AbstractAckHandler<S012AckPayload> {

    /** 記錄 ACK 到 mqtt_message_log 的服務 */
    private final MqttMessageLogService logService;

    /** 存取 door_access_info 的 Repository（以 TID 鏈結狀態） */
    private final DoorAccessInfoRepository doorAccessInfoRepository;
    private final HmiDisplayTaskRepository hmiRepo;
    public S012AckHandler(ObjectMapper objectMapper,
     ...
     * @param ack    已解析的 S012AckPayload
     */
    @Override
    protected void process(String system, String topic, S012AckPayload ack) throws Exception {
        ...
        log.info("[S012] 收到關門驗證 ACK：TID={}, RESULT={}, STAFFS={}", tid, result, staffList);
        ...
        try {
            ...
            if (opt.isEmpty()) {
                // 正常情況：送出 S012 時會先 savePending() 建一筆 PENDING
                log.warn("[S012] 找不到對應的 door_access_info 紀錄（可能未入列或已清除），tid={}", tid);
                return;
            }
            ...
            // 不在此處改 writeback_status，維持 WAITING，交給 Writer 去回寫 PLC
            boolean updated = doorAccessInfoRepository.update(row);
            if (!updated) {
                log.warn("[S012] 更新 door_access_info 失敗，tid={}", tid);
            } else {
                log.info("[S012] 已更新 door_access_info：tid={}, status={}, ackResult={}",
            ...
            log.error("[S012] 更新 door_access_info 發生例外，tid={}, err={}", tid, e.getMessage(), e);
        }
    }
    ...
    @Override
    protected Class<S012AckPayload> getAckType() {
        return S012AckPayload.class;
    }
}
