...
public class TtReportMonitor {
    ...
    private Map<String, String> dicId = new HashMap<>();
    ...
    public void init() {
        ...
        for (var entry : byDevice.entrySet()) {
            ...
            Long locationPoint = list.get(0).getLocationPoint();
            // 找 Index / TransferNo
            ...
            String baseAddr = list.stream()
                    ...
                    .min(Comparator.comparingInt(TtReportMonitor::plcWordToInt))
                    .orElse(null);
            if (baseAddr == null) {
            ...
            DeviceRuntime rt = new DeviceRuntime(
                    ...
                    list, Integer.parseInt(lastIndex), locationPoint, deviceArea
            );
    ...
    @Scheduled(fixedRate = FIXED_RATE_MS, initialDelay = 500)
    public void tick() {
    ...
    private void processOneDevice(DeviceRuntime d) {
        ...
        ContainerMain cc = null;
        if (d.locationPoint != null && d.locationPoint > 0) {
            LocationTracking qq = locationTrackingRepository.findByLocationPointId(d.locationPoint).orElse(null);
            cc = qq != null ? containerMainRepository.findById(qq.getContainerMainId()).orElse(null) : null;
        }
        String remarkId = cc != null ? cc.getAliasCode() : "";

        if (remarkId.isBlank()) {
            switch (d.deviceName) {
                case "Strapping#1" -> {
                    remarkId = dicId.getOrDefault("WorkingBeam#8", "");
                }
                case "Strapping#2" -> {
                    remarkId = dicId.getOrDefault("WorkingBeam#6", "");
                }
                case "CV1" -> {
                    remarkId = dicId.getOrDefault("Strapping#1", "");
                }
                case "CV2", "CV3", "CV4" -> {
                    remarkId = dicId.getOrDefault("CV1", "");
                }
                case "CV5" -> {
                    remarkId = dicId.getOrDefault("Strapping#2", "");
                }
                case "CV6", "CV7", "CV8", "CV9", "CV10" -> {
                    remarkId = dicId.getOrDefault("CV5", "");
                }
                default ->  {
                    remarkId = dicId.getOrDefault(d.deviceName, "");
                }
            }
        }
        if (!remarkId.isBlank()) {
            dicId.put(d.deviceName, remarkId);
        }
