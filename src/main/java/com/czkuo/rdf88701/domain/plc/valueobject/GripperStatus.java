...
/**
 * GripperStatus
 * - 封裝 Gripper 裝置的狀態 Word 資料（W1263）
 * - 對應格式：wxyz rrrr ssss ssss
 *   - ssss ssss: Gripper Device Status (DEC)
 *       - 1: Idle
 *       - 2: Processing
 *       - 3: Complete
...
public class GripperStatus {
    ...
    private int gripperStatus;      // ssss ssss
    ...
    /**
     * 由 Word 值解析出 GripperStatus 物件
     */
    public static GripperStatus fromWord(int word) {
        GripperStatus status = new GripperStatus();
        status.raw = word;
        status.gripperStatus = word & 0x00FF;                      // 最右 8 bit
    ...
    public int toRaw() {
        int word = 0;
        word |= (gripperStatus & 0xFF);               // bits 0~7
    ...
    public boolean isIdle() {
        return gripperStatus == 1 && runningSubStatus == 1;
    }

    public boolean isBusy() {
        return gripperStatus == 2 || runningSubStatus == 2 || runningSubStatus == 3 || runningSubStatus == 4;
    }

    public boolean isComplete() {
        return gripperStatus == 3;
    }
